...
C   (submitted to PRL).
      ...
      implicit none
      integer i,f
      double precision alphas,az,aw,at,al,alh,dgamma,const
      double precision dmhmt,cqcd1,cqcd2,cqcd3,rho2m4
...
C   first order iteration:
      ...
      delrwh = al*dgamma + 3*mmu**2/mw2/10.d0
      rhohat = 1 + 3*xt/4
...
C   (the second term absorbs the W-propagator effect in Delta r 
C   which is the convention adopted since 2011):

      delrwh = al*dgamma + 3*mmu**2/mw2/10.d0 + 
     .       al/4/sinhat*(6 + dlog(cos2tw)/sin2tw*
     .       (7/2.d0 - 5*sin2tw/2 - sinhat*(5 - 3*rhohat/2))) 

      if ((flagzp.ne.0).and.(flinit.eqv..false.))
     .     delrwh = delrwh - 3*ratg21**2*al/2/sinhat/coshat*
     .     eps2_L(1)*eps2_L(2)*dlog(mzp2/mw2)/(mzp2/mw2 - 1)
...
C   Leading O(alpha alpha_s**2 mt**2 and alpha alpha_s**3 mt**2) 
C   correction to rhohat:
      
      if ((fas2mt.eqv..true.).and.(ffermi.eqv..false.)) delrho = delrho 
     .     - alh*at**2/4/sinhat/coshat*rattz2*cqcd2
     .     - alh*at**3/4/sinhat/coshat*rattz2*cqcd3
...
C   J. Fleischer, O.V. Tarasov and F. Jegerlehner, PLB 319 (1993) 249.
          ...
          rho2 = 0.d0
            b1 = 1/ratth2
            b2 = b1**2
        rho2m4 = 25 - 4*b1 + (b2 - 12*b1 - 12)/2*dlog(b1)
     .         + (b1 - 2)/2/b1*pi2 + (b1 - 4)/2*dsqrt(b1)*gftj(b1)
     .         - (3*b2 - 12*b1 + 15 - 6/b1)*fftj0(b1) 
     .         + 3*(b2 - 6*b1 + 10)*fftj1(b1)
      if ((fa2mt4.eqv..true.).and.(fa2mt2.eqv..false.)) rho2 = rho2m4
...
C   mt^6 term (hep-ph/0302275) including a residual term - x1*x2 
C   (in the last line of rho3) from using
C
C   a) gauge coupling normalization
C   b) rhohat in expanded form (at this point in the program)

      dmhmt = mh/mt - 1.d0
      
      rho3 = (95.92d0 - 111.98d0*dmhmt + 8.099d0*dmhmt**2 
     .     + 9.36d0*dmhmt**3 + 7.27d0*dmhmt**4 - 15.6d0*dmhmt**5)/3
     .     - rho2m4

      if (ffermi.eqv..false.) delrho = delrho
     .     + 3*(alh/16/sinhat/coshat*rattz2)**2*rho2
     .     + 3*(alh/16/sinhat/coshat*rattz2)**3*rho3

c      write(7,*) alh/sinhat/coshat
c      write(7,*) 3*(alh/16/sinhat/coshat*rattz2)
c      write(7,*) 3*(alh/16/sinhat/coshat*rattz2)**2*rho2m4
c      write(7,*) 3*(alh/16/sinhat/coshat*rattz2)**2*(rho2 -  rho2m4)
c      write(7,*) 3*(alh/16/sinhat/coshat*rattz2)**3*rho3
c      write(7,*)

C   alpha_s mt^4 term (hep-ph/0302275):

      rho2a = 157.295d0 + 112.d0*dmhmt - 24.73d0*dmhmt**2 
     .      + 7.39d0*dmhmt**3 - 3.52d0*dmhmt**4 + 2.06d0*dmhmt**5
     .      + 16*rho2m4

      if ((fas2mt.eqv..true.).and.(ffermi.eqv..false.)) delrho = delrho 
     .     + at*(alh/16/sinhat/coshat*rattz2)**2*rho2a

C      delrho = delrho + at*(xt*rhohat)**2/16*rho3
C
C            write (7,*) at*(xt*rhohat)**2/16*(157.295d0 + 112.d0*dmhmt 
C     .           - 24.73d0*dmhmt**2 
C     .           + 7.39d0*dmhmt**3 - 3.52d0*dmhmt**4 + 2.06d0*dmhmt**5)
C
C            write (7,*) rho2a, 157.295d0 + 112.d0*dmhmt 
C     .           - 24.73d0*dmhmt**2 
C     .           + 7.39d0*dmhmt**3 - 3.52d0*dmhmt**4 + 2.06d0*dmhmt**5,
C     .           - 16.d0*rho2
...
C   be done in the O(alpha mt**2) term.
     ...
     .             + 3*xt/4*rhohat*(1 + xt*rhohat/4*rho2)
            ...
            delrho = delrho +  3*(xt/4*rhohat)**2*rho2
     .                      +  3*(xt/4*rhohat)**3*rho3
     .                      + at*(xt/4*rhohat)**2*rho2a
         endif
         if (fasmt2.eqv..true.) delrho = delrho - at*xt*rhohat*cqcd1
         if (fas2mt.eqv..true.) delrho = delrho - 
     .        at**2*xt*rhohat*(cqcd2 + at*cqcd3)
      endif
...
C old implementaion of STU
C
C      if (fobliq.eqv..true.) then
C         delrwh = delrwh + alpha/4/sinhat*SWpar
C         delrho = delrho + alphat
C     .          * (Tpar + (cos2tw*SWpar - Spar)/4/sinhat/coshat)
C      endif
C
C  enhanced three-loop effect from complex pole treatment.
C
C      delrho = delrho - 0.72d-4
C
C to get agreement with Czakon et al fitting formulas:
C
CCC    delrwh = delrwh - 2.56d-4
CCC    delrho = delrho - 2.28d-4

C     delrwh = delrwh - 1.58d-4
C     delrho = delrho - 2.07d-4

cc      delrwh = delrwh - 0.73d-4 ! to get agreement with Degrassi et al (1411.7040):
cc      delrho = delrho - 2.94d-4 ! to get agreement with two-loop program of Ayres Freitas

C to get agreement with two-loop program of Ayres Freitas

      delrho = delrho - 2.06d-4 ! see also adjustments in kappa.f and rho.f

CCC   my delrho = rhohat - 1 differs from Y_MSbar = delrhohat in 
CCC   Degrassi, Fanchiotti, Sirlin NPB 1990 which is mulitplied by 1/rhohat
CCC   and which is why their relation 
CCC   delrzh = (delrwh - delrho)/(1 - delrho)  
CCC   is modified to read

      delrzh = delrwh - delrho*(1 - delrwh)

C  The definitions of SWpar and Spar are written with alphat, but since
C  we write delrwh (delrzh) in resummed form we have to use alpha below.
C  Also, note that delrzh is written without an index Z by Sirlin et al.

      if (fobliq.eqv..true.) then
         delrwh = delrwh + alpha/4/sinhat*SWpar
         delrzh = delrzh + alpha/coshat*(Spar/4/sinhat - cos2tw*Tpar)
      endif
...
C     write(7,*) delr,1 - a0**2/mw2/sin2tw
